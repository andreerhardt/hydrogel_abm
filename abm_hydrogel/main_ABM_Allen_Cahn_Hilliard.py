...
def evolve(old_q,t,dt,node1_x,node1_y,node2_x,node2_y):
...
vertical_dipole_no      = ndipole.count(1)
horizontial_dipole_no   = ndipole.count(2)
diagonal_dipole_no      = ndipole.count(3) + ndipole.count(4)

dipole_no               = [[0,0.0,vertical_dipole_no,horizontial_dipole_no,diagonal_dipole_no]]

q, E_free = evolve(old_q,t,dt,node1_x,node1_y,node2_x,node2_y)
deform = abm.deformation(q)

save_complete_state(output_folder + 'initial',mesh,q,abm,t)

# runing the main time loop by stretching the hydrogel/elastic solid
for i in range(1,n_steps+1):  
    print("iteration ",i," of ",n_steps)
    """
    moving the cells and determining the new position
    applying point load the the ECM and determining the dipoles, nodes and deformation
    """
    move_list = abm.move()
    """
    solve the FEA
    ...
    print("# subiteration 1")
    q, E_free = evolve(old_q,t,dt,node1_x,node1_y,node2_x,node2_y)
    for j in range(1,2):
        q, E_free = evolve(q,t,dt,node1_x,node1_y,node2_x,node2_y)
        
    """determining the new deformation"""
    deform_new = abm.deformation(q)
    """checking the satisfaction of the cells and moving the unsatisfied cells"""
    abm.update_agents(deform,deform_new,move_list)
    """running the final FEA"""
    (n1list,n2list,clist,node1_x,node1_y,node2_x,node2_y,ndipole) = abm.get_dipole_direction_point_source()

    print("# subiteration 2")
    q, E_free = evolve(old_q,t,dt,node1_x,node1_y,node2_x,node2_y)
    for j in range(1,2):
        q, E_free = evolve(q,t,dt,node1_x,node1_y,node2_x,node2_y)
    ...
    """save FEA ouput"""
    if i in range(1,n_steps+1,round(n_steps/100)):
        save_complete_state(output_folder + 'state'+str(i),mesh,q,abm,t)
