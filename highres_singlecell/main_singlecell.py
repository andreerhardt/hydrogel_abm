...
N_Refine      = abm_pars["N_Refine"]
delta0        = abm_pars["delta0"]
...
t     = 0.0
mesh0 = RectangleMesh(Point(0.4,0.4),Point(0.6,0.6),20,20,"left/right")
ff    = project(inner(force,force),FunctionSpace(mesh0,'CG',1))

# iteratively refine isolines
for i in range(N_Refine):
    marker1  = mark_diff(mesh0,ff,abs(f0)/5) # mark isolevels
    if i==0:
        mesh1,level1 = refine_and_adapt(marker1,mesh0) # refine and count levels
    else:
        mesh1,level1 = refine_and_adapt(marker1,mesh0,level0)
    # set variables for next iterations and interpolate function
    mesh0  = mesh1
    level0 = level1
    ff = project(inner(force,force),FunctionSpace(mesh0,'CG',1))
...
save_state(mesh, q, output_folder + 'state'+str(0), t)
...
for i in range(1, n_steps+1):
    ...
    #if (i<15):
    f0 = (i/n_steps) * traction / (2*np.pi*d0)
    #else:
    #    f0 = traction / (2*pi*d0)
